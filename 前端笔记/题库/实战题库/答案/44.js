...
console.log( data.reduce((sum, d) => sum + d[2], 0) / data.length );
...
function getTurnPoints(data, start, end) {
    ...
    for (let i = 1; i < currentData.length - 1; i++) {
        const close = currentData[i][2]; // 收盘价
        const prevClose = currentData[i - 1][2]; // 前一天的收盘价
        const nextClose = currentData[i + 1][2]; // 后一天的收盘价
...
console.log( getTurnPoints(data, '2013/2/1', '2013/2/5') ); // []
console.log( getTurnPoints(data, '2013/1/24', '2013/1/31') ); // ['2013/1/25']
